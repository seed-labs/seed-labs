...
\texttt{Labsetup/emulator.py}. The script intentionally keeps only a few command
line options:
...
Changing \texttt{--amplifier-hosts} is a convenient way to change the
amplification factor, so old lab reports cannot simply reuse one fixed value.
...
produce approximately \(N\) replies. Figure~\ref{fig:smurf-attack} illustrates the attack.
...
\begin{lstlisting}
Attacker  --spoofed ICMP echo request-->  AS-152 broadcast address
Amplifier hosts  --ICMP echo replies-->  Victim
\end{lstlisting}
...
amplifier hosts. On each of these nodes, run suitable commands to inspect their
...
broadcast address is used? Which theory from Section 3 explains this behavior?
...
ip = IP(src=victim_ip, dst=broadcast_ip)
icmp = ICMP(type=...)
payload = b"..."
...
\subsection{Task 5: Comparing Smurf and Fraggle}

The objective of this task is to identify the common attack pattern and the
protocol-specific differences.

Use \texttt{tcpdump} on the victim and on one amplifier host while running both
attacks. For the Fraggle attack, students can also run \texttt{tcpdump} on an
amplifier host to observe the incoming spoofed UDP request:

\begin{lstlisting}
# tcpdump -n -i any udp port 7000
\end{lstlisting}

Compare the two attacks using the following questions:

\begin{itemize}[noitemsep]
\item Which fields are spoofed in the packets sent by the attacker?
\item Which packets are sent to the broadcast address?
\item Which packets are sent to the victim?
\item What role does the amplifier LAN play?
\item Which parts of the attack are the same for Smurf and Fraggle?
\item Which parts are protocol-specific?
\end{itemize}


\subsection{Task 6: Defense 1 -- Disabling Directed Broadcast}
...
\subsection{Task 7: Defense 2 -- Disabling ICMP Broadcast Replies}
