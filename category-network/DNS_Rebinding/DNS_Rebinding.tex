...
\begin{figure}[htb]
\centering
\includegraphics[width=0.9\textwidth]{\rebindingFigs/environment_setup_container.pdf}
...
will be using VM, and the others are all containers. Please download the 
...
The firewall blocks
all traffic going to the \texttt{10.8.0.0/24} network. 
This way, outside machines cannot 
access the machines on the home network. 
...
\paragraph{Remove route on the user VM.}
When we create a network using docker, 
the host machine (i.e., the user VM) will be automatically
attached to the network and is given the IP address 
\texttt{.1}. In our setup, we create two networks, so 
the user VM will be connected to both networks.
If we run the \texttt{ip route} command, we will see the following: 

\begin{lstlisting}
$ ip route 
10.8.0.0/24 dev br-77cfd5c95222 proto kernel scope link src 10.8.0.1
10.9.0.0/24 dev br-97924f33175f proto kernel scope link src 10.9.0.1
\end{lstlisting}

We can see that the user VM can directly access the \texttt{10.9.0.0/24} network.
That is different from what is depicted in Figure~\ref{dns:fig:rebind_environment}.
To simulate the same setup, we will remove the direct route to
the \texttt{10.9.0.0/24} network, and then add a new 
routing entry to use \texttt{10.8.0.2} to reach the outside network. 
We will execute the following commands on the user VM.
 

\begin{lstlisting}
$ sudo ip route del 10.9.0.0/24
$ sudo ip route add 10.9.0.0/24 via 10.8.0.2
\end{lstlisting}
...
\texttt{10.9.0.22} and \texttt{10.9.0.21}, respectively. 
