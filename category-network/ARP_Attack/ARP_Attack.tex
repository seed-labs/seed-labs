...
In this lab, students need to have 3 machines. We use 
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.8\textwidth]{Figs/ARP_onelan.pdf}
\end{center}
...
\paragraph{Privileged mode.}
For the Host M container, we used the \texttt{privileged} mode; without it,
we will not be able to run \texttt{sysctl} to enable/disable the 
IP forwarding. The use of this mode is specified in the Compose file: 
