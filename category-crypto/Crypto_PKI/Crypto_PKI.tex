...
Many websites have different names. For example, \texttt{www.example.com},
\texttt{example.com}, \texttt{example.net}, and \texttt{example.org} 
are all pointing to the same web server. This causes a problem
to the certificate, because in the common name field, we can
only put one name. To solve this problem, X509 adds an 
\texttt{subjectAltName} extension, allowing alternative names 
to be put in the extension field. 

You should add two alternative names to you server. 
You can achieve that by adding the following option to the \texttt{openssl req} command. 
It should be noted that the list must also include the one used in the common name field; 
otherwise, the name specified in the common name field will not be accepted as a valid name.
...
\paragraph{Copy the extension filed.}
The default setting in \texttt{openssl.cnf} does not allow
