...
int main(int argc, char const *argv[])
{
    struct
    {
        ...
        {.port = 5000, .binary = "padding_oracle_L1"},
        {.port = 6000, .binary = "padding_oracle_L2"}
      };

    int server_fds[N] = {
        socket_bind(programs[0].port), // for padding_oracle_L1
        socket_bind(programs[1].port)  // for padding_oracle_L2
    };
    ...
    while (true)
    {
        ...
        unsigned server_idx;
        int socket_fd = select_accept(server_fds, N, (struct sockaddr *)&address, (socklen_t *)&addrlen, server_idx);
        if (socket_fd < 0)
        {
