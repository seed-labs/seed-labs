...
Your job is to construct a message \texttt{ptext2} and ask Bob to encrypt it
and give you the ciphertext.
Your objective is to use this opportunity to figure out
whether the actual content of \texttt{ptext1} is \texttt{Yes} or \texttt{No}.
...
\begin{lstlisting}
$ nc 10.9.0.80 3000
ciphertext 1: 1baf79c133609584792d34120807cf1a
iv 1        : 5b6ac1907f992f11c81310ae81e3b017
iv 2        : c9302ed6046bd711a228db5d68c8eaaf

plaintext 2 :
\end{lstlisting}

After showing \texttt{ctext1}, \texttt{iv1} and \texttt{iv2},
the oracle will require you to input \texttt{ptext2} as a hex string.
Then it encrypts the \texttt{ptext2} with \texttt{iv2}
and outputs the new ciphertext.
You can try different \texttt{ptext2}s and terminate the oracle
by pressing \text{Ctrl+C}.
...
Typically, encryting messages using AES with CBC mode requires to
pad the message with PKCS\#5 padding scheme.
Some implementation of the deciphers verify
if the padding is valid after decrypting the ciphertext,
and throws an error if the padding is corrupted.

In this task, we provide such a padding oracle hosted on port \texttt{5000}.
The oracle provides a ciphertext.
You may construct your own ciphertext and ask the oracle to check
if the ciphertext has a valid padding.

Your objective is to \begin{enumerate}
  \item Find the length of the plaintext.
  \item Find the actual content of the plaintext.
\end{enumerate}

There is a python script named \texttt{padding\_oracle\_attack.py}
included in the \texttt{Labsetup.zip}.
You may use the skeleton to develope your own program.
