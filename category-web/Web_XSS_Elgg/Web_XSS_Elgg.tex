...
In our container setup, the MySQL database stores data 
in the \texttt{mysql\_data} folder (inside \texttt{Labsetup})  
on the host machine (the 
folder will be created after the MySQL container runs once). 
This way, even if the MySQL container is destroyed, the 
data in the database are still kept. If you do want to start from a clean
